...
class DynamaskSetExplainer(BaseExplainer):
    ...
    def _attribute_multiple(self, x, mask_in):
        ...
        def f(x_in, mask_in):
            ...
            num_sample, num_times, _ = x_in.shape
            x_in = x_in.permute(0, 2, 1)
            mask_in = mask_in.permute(0, 2, 1)
            out = self.base_model(
                x_in, mask_in, return_all=True
            )  # (num_sample, num_state=1, num_time)
            out = self.base_model.activation(out).reshape(num_sample, num_times)
            if self.num_class == 1:
                # stack
                out = torch.stack([1 - out, out], dim=2)
            else:
                out = out.reshape(num_sample, self.num_class, num_times).permute(
                    0, 2, 1
                )
