            ...
            for t in range(num_timesteps):
                ...
                for n in range(window_size):
                    ...
                    for f in range(num_features):
                        ...
                        mask_hat_in[:, :, f, time_past : t + 1] = 1  # Values exist
            ...
            for t in range(num_timesteps):
                ...
                for n in range(window_size):
                    ...
                    for f in range(num_features):
                        ...
                        last_value = x_hat_in[
                            :, :, f, time_past - 1 : time_past
                        ]  # Get last value
                        x_hat_in[:, :, f, time_past : t + 1] = last_value.repeat(
                            1, 1, time_forward
                        )
...
class WinITSetZeroExplainer(BaseExplainer):
    """
    The explainer for our method WinIT
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.log = logging.getLogger(WinITSetZeroExplainer.__name__)
        if len(kwargs):
            self.log.warning(f"kwargs is not empty. Unused kwargs={kwargs}")
    ...
    def attribute(self, x, mask=None):
        """WinIT attribution using zero for counterfactuals"""
        self.base_model.eval()
        self.base_model.zero_grad()
        ...
        with torch.no_grad():
            ...
            for t in range(num_timesteps):
                ...
                for n in range(window_size):
                    ...
                    # Instead of generating counterfactuals, directly zero out
                    for f in range(num_features):
                        x_hat_in = x[:, :, : t + 1].unsqueeze(0)  # (1, bs, f, time)
                        mask_hat_in = mask[:, :, : t + 1].unsqueeze(0)
                        
                        # Zero out the feature values
                        x_hat_in[:, :, f, time_past : t + 1] = 0
                        mask_hat_in[:, :, f, time_past : t + 1] = 0  # Values doesn't exist
...
class WinITSetZeroLongExplainer(BaseExplainer):
    """
    The explainer for our method WinIT
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.log = logging.getLogger(WinITSetZeroLongExplainer.__name__)
        if len(kwargs):
            self.log.warning(f"kwargs is not empty. Unused kwargs={kwargs}")
...
class WinITHiddenExplainer(BaseExplainer):
    """
    The explainer for our method WinIT
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.log = logging.getLogger(WinITHiddenExplainer.__name__)
        if len(kwargs):
            self.log.warning(f"kwargs is not empty. Unused kwargs={kwargs}")
    ...
    def _model_predict(self, x, mask=None, timesteps=None):
        ...
        """
        assert timesteps is not None
        p = self.base_model.encoding(x, mask, timesteps, return_all=False)
        # if self.base_model.num_states == 1:
        #     # Create a 'probability distribution' (p, 1 - p)
        #     prob_distribution = torch.cat((1 - p, p), dim=1)
        #     return prob_distribution
